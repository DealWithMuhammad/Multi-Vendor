...
export function Page() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <aside
        className={`bg-white w-64 min-h-screen flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed lg:relative lg:translate-x-0 z-50`}
      >
        ...
        <nav className="flex-1 p-4">
          <ul className="space-y-2">
            {["Dashboard", "Orders", "Wishlist", "Messages", "Settings"].map(
              (item) => (
                <li key={item}>
                  <a
                    href="#"
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      {orders.map((order) => (
                        <div
                          key={order.id}
                          className="flex flex-col md:flex-row md:items-center justify-between space-y-2 md:space-y-0 bg-white p-4 rounded-lg shadow"
                        >
